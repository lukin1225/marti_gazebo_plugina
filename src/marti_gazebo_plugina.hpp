...
class CarGazeboPlugin : public gazebo::ModelPlugin {
 ...
 private:
  ...
  double steer = 0;
  double velocity = 0;
  const double wheelbase_length = 0.4;
  const double front_wheelbase_width = 0.31;
  const double rear_wheelbase_width = 0.31;
  const double wheel_diameter = 0.10;
  const double max_speed = 3;
  const double max_turn_angle = M_PI/4.0;
  AckermannModel car_model = {0.31, 0.31};
