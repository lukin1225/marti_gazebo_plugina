...
void CarGazeboPlugin::Load(gazebo::physics::ModelPtr model,
  ...
  // fake_car code here
  {
    ...
    fl_str_joint = get_joint("aurrekoezk_gurp_steer_joint");

    jc->SetPositionPID(fl_str_joint->GetScopedName(), fl_pid);

    // front right str
    fr_pid = gazebo::common::PID(1, 0, 0);
    fr_str_joint = get_joint("aurrekoesk_gurp_steer_joint");
    jc->SetPositionPID(fr_str_joint->GetScopedName(), fr_pid);

    fl_axle_joint = get_joint("aurrekoezk_gurp_joint");
    fr_axle_joint = get_joint("aurrekoesk_gurp_joint");

    // back left speed
    bl_pid = gazebo::common::PID(0.1, 0.01, 0.0);
    bl_axle_joint = get_joint("atzekoezk_gurp_joint");
    jc->SetVelocityPID(bl_axle_joint->GetScopedName(), bl_pid);

    br_pid = gazebo::common::PID(0.1, 0.01, 0.0);
    br_axle_joint = get_joint("atzekoesk_gurp_joint");
